...
public class Editor implements MqttCallbackExtended {
    ...
    public void finalizar() {
        if (client != null && client.isConnected()) { //Se cliente nÃ£o for nulo e estiver conectado
            try {
    ...
    public synchronized void publicar(String topico, byte[] mensagem, int qos, boolean retained) {
        try {
            if (client.isConnected()) { //Se o cliente estiver conectado ao broker MQTT
                ...
                System.out.println(String.format("InformaÃ§Ã£o publicada no tÃ³pico %s com sucesso", topico));
            } else { //Se estiver desconectado
                System.out.println("NÃ£o foi possivel publicar no tÃ³pico " + topico+", pois o editor estÃ¡ desconectado");
            }
        } catch (MqttException ex) { //Se houver erros ao publicar o topico
            System.out.println("Erro ao publicar o tÃ³pico:" + topico + ": " + ex);
        }
    }

     /**
     * Método que sinaliza se a conexÃ£o com o broker foi perdida
     * @param thrwbl 
     */
    @Override
    public void connectionLost(Throwable thrwbl) {
        System.out.println("Editor: ConexÃ£o com o broker perdida -" + thrwbl);
    }
